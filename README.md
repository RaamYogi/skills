...
## Running when Maven or Java is not already installed

- A helper PowerShell script `scripts/run-app.ps1` can download Maven 3.9.6 into `.mvn/apache-maven-3.9.6` and run `spring-boot:run` without needing a global Maven install. Run it via `powershell -ExecutionPolicy Bypass -File scripts\run-app.ps1`.
- `JAVA_HOME` must point at a Java 17+ installation or you can install a JDK yourself. The script will fail early with a clear error if it cannot find Java, so install OpenJDK 17, set `JAVA_HOME`, and re-run.
